...
The core runtime, Git worktree topology, balanced recursive fan-out, ownership validation, canonical WIT parsing plus Quackery's one-export world policy, result commits, recursive join, role model routing, cache grouping/telemetry, and text graph are implemented and testable without OpenCode. The current WIT policy intentionally supports local interfaces; broader package-qualified worlds are future work. Live OpenCode execution and actual provider cache hits depend on the configured provider and repository and must be validated in an installed host.
